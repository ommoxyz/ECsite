...
public class MyPageDAO {
	...
	public MyPageDTO getMyPageUserInfo(String item_transaction_id, String user_master_id) {
		...
		//item_info_transactionをiit、user_buy_item_transactionをubitと省略するために再定義している
		//ORDER BY で入れた値を並び替え
		String sql="SELECT iit.item_name, ubit.total_price, ubit.total_count,"
				+ "ubit.pay FROM user_buy_item_transaction ubit LEFT JOIN item_info_transaction iit "
				+ "ON ubit.item_transaction_id = iit.id WHERE ubit.item_transaction_id = ? AND "
